...
import { fetchAreas } from '@/api/common/areas';
import {
  getData,
} from '@/utils/dom';
...
export default {
  ...
  methods: {
    async getAreas() {
      ...
      const areaList = res.data.rows;
      const areaListMap = {};
      areaList.forEach((area) => {
        const initial = area.initial.toLocaleUpperCase();
      ...
      const ret = [];
      for (const key in areaListMap) {
        const val = areaListMap[key];
