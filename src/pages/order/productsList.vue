<template>
  <div class="container">
    <div class="header">
      <mt-header title="商品列表" ref="header" class="header">
      </mt-header>
    </div>
    <van-search
      @clear="resetSearch"
      ...
      v-model="search.text"
      show-action
    ><div slot="action" @click="onSearch(false)">搜索</div></van-search>
    <div class="content" ref="scrollWrap">
      ...
      >
        <div class="car-list_box">
          <div class="car-list_item van-hairline--bottom" v-for="(car, index) in currentList" :key="index" @click="selectProduct(car)">
            <div class="car-thumb">
            ...
            <div class="car-info" v-if="options.lx === 'news'">
              <p class="title">{{car.title}}</p>
              <!-- <p class="info">{{'xxx'+'/'+'12345678911'}}</p> -->
              <p class="info">{{car.price}}万/<span class="blue">首付{{car.payment}}万</span></p>
              <p class="info">{{car.year}}</p>
            </div>
            <div class="car-info" v-if="options.lx === 'olds'">
              <p class="title">{{car.name}} </p>
              <!-- <p class="info">{{'xxx'+'/'+'12345678911'}}</p> -->
              <p class="info">{{car.price}}万/<span class="blue">首付{{car.payment}}万</span></p>
              <p class="info">{{car.year}} </p>
            </div>
            <div class="car-info" v-if="options.lx === 'rentings'">
              <p class="title">{{car.title}} </p>
              <!-- <p class="info">{{'xxx'+'/'+'12345678911'}}</p> -->
              <p class="info">押金:{{car.price}}元</p>
              <p class="info">租金:{{car.payment}}元</p>
            </div>
            <div class="car-info" v-if="options.lx === 'cosm'">
              <p class="title">{{car.title}} </p>
              <!-- <p class="info">{{'xxx'+'/'+'12345678911'}}</p> -->
              <p class="info">{{car.price}}元</p>
              <!-- <p class="info">{{car.year}}</p> -->
            </div>
          </div>
        </div>
