<template>
  <div class="container bg_white">
    <mt-header title="入库列表" ref="header" class="header">
      <a @click.stop="$router.go(-1)" slot="left">
        <mt-button icon="back"></mt-button>
      </a>
      <!-- <span slot="right">
        <icon name="add" scale="1.8" color="#333" @click.native="typein"></icon>
      </span> -->
      <!-- 增加入库功能 -->
    </mt-header>
    ...
    才能触发quick-loadmore的上拉加载功能 -->
      <quick-loadmore ref="vueLoad" :bottom-method="handleBottom" :bottom-status-change="handleBottomStatusChange"
        ...
        <div class="item van-hairline--bottom" v-for="item in currentList" :key="item.id" @click="typein(item)">
