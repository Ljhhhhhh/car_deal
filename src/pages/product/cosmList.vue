<template>
  <div class="header">
    <mt-header title="洗美护管理" ref="header" class="header">
      <router-link to="/product" slot="left">
        <mt-button icon="back" style="color:black"></mt-button>
      </router-link>
      <span slot="right" class="head-right">
        <icon name="add" scale="1.8" color="#333" @click.native="newCosm"></icon>
      </span>
    </mt-header>
    <van-search  @clear="resetSearch" background="#f5f4f7" placeholder="请输入搜索关键词" ref="search" v-model="search.text"></van-search>
    <div class="content" ref="scrollWrap">
      ...
      >
        <div class="car-list_box">
          <div class="car-list_item van-hairline--bottom" v-for="(car, index) in currentList" :key="index"   @click="newCosm(car)">
            <div class="car-thumb">
